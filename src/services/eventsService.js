...
/** Admin-only in practice: RLS only returns every event when the caller is an admin. */
...
function mapEventsError(error) {
  ...
  if (message.includes('events_title_length')) {
    return 'Title must be between 1 and 200 characters.';
  }
  if (message.includes('events_description_length')) {
